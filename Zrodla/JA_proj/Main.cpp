...
/// INICJALIZACJA ZMIENNYCH ////////////////////////////////////
	...
	threadsEnd = new bool[threadsCount];

	

/// INICJALIZACJA TABLCY threadsEnd;
...
/// Dllka cpp //////////////////////
	if (cppDllOn) {
		for (int i = 0; i < threadsCount; i++)
		{
			threadsArray[i] = std::thread(MyClass::Foo, tabA, tabB, tabC, threadsRange[i], threadsRange[i + 1], x, y, z,&threadsEnd[i]);
		}
		...
		do {
			 temp = true;
			std::cout << "Waiting for threads number:";
			for (int i = 0; i < threadsCount; i++) {
				if (!threadsEnd[i]) std::cout << i << " ,";
				temp = temp&&threadsEnd[i];
			}
			std::cout << "\n\n";
		} while (!temp);
